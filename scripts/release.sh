...
if [[ $DEVICE == marlin || $DEVICE == sailfish || $DEVICE == taimen || $DEVICE == walleye ||
  ...
  FP4="true"
elif [[ $DEVICE == instantnoodlep || $DEVICE == kebab || $DEVICE == lemonade || $DEVICE == lemonadep ]]; then
  : # Do nothing, for now.
else
...
elif [[ $DEVICE == barbet || $DEVICE == FP4 || $DEVICE == instantnoodlep || $DEVICE == kebab || $DEVICE == lemonade ||
  $DEVICE == lemonadep ]]; then
...
if [[ $DEVICE == taimen || $DEVICE == walleye || $DEVICE == blueline || $DEVICE == crosshatch ||
  ...
  $DEVICE == oriole || $DEVICE == raven || $DEVICE == FP4 || $DEVICE == instantnoodlep || $DEVICE == kebab ||
  $DEVICE == lemonade || $DEVICE == lemonadep ]]; then
